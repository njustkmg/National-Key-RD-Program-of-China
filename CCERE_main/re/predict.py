...
def _get_predict_instance(cfg):
    ...
    if flag == 'y' or flag == 'yes':
        ...
        head_type = 'BREED'
        tail_type = 'PHENOTYPE'
        # sentence = '玉米自交系铁7922株型紧凑,茎秆坚韧,品质优良,配合力高,抗逆性强,遗传性状稳定,适应范围广,是优异的玉米自交系和种质资源。'
        # head = '铁7922'
        # tail = '品质优良'
        # head_type = 'BREED'
        # tail_type = 'PHENOTYPE'
        # sentence = '结果表明,在250~400Gy范围内,随着剂量的增加,小麦种子的出苗率、成株率降低,不育率升高,株高、每穗小穗总数、每穗结实小穗数、每穗粒数减少,千粒重降低。'
        # head = '剂量'
        # tail = '每穗小穗总数'
        # head_type = 'ENVIRONMENT'
        # tail_type = 'PHENOTYPE'
...
def main(cfg):
    ...
    # cfg.lm_file = r'E:\PyCharmProject\FoundationModel-Agriculture\knowledge\ner\wheat'
    # cfg.num_relations = 53
    # cfg.data_path = r'E:\PyCharmProject\FoundationModel-Agriculture\knowledge\re\origin_wheat_1500'
    # cfg.fp = r'E:\PyCharmProject\FoundationModel-Agriculture\knowledge\re\wheat.pth'
    print(cfg.pretty())
    ...
    with torch.no_grad():
        ...
        # print(rels.keys())
        # y_pred_argmax = y_pred.argmax().item()
        # print(y_pred_argmax)
        prob_rel = list(rels.keys())[y_pred.argmax().item()]
        print('head_type:' + data[0]['head_type'] + ' tail_type:' + data[0]['tail_type'])
